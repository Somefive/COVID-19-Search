        ...
        {this.state.search.entries.map((entry, idx) => {
          return <div className="search-entries" key={idx} onMouseEnter={() => this.setState({hoverEntry: entry})}>
            <div className="entry-title">
              {entry.urls.length > 0 && <a href={entry.urls[0]} target="_blank" rel="noopener noreferrer">{this.highlightedText(entry.title)}</a>}
              {entry.urls.length === 0 && this.highlightedText(entry.title)}
            </div>
